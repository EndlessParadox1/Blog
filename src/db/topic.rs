...
pub async fn create(client: &Client, frm: &Topic, username: String) -> Result<bool> {
    ...
    let sql = "INSERT INTO topics (title, summary, html, markdown, dateline, writer, is_del) VALUES ($1, $2, $3, $4, $5, $6, false)";
    let n = super::execute(
        client,
        sql,
        &[
...
pub async fn list_latest(client: &Client, username: String) -> Result<Vec<model::Topic>> {
    let sql = "SELECT id, title, summary, dateline from topics WHERE writer = $1 AND is_del = false LIMIT 10";
...
pub async fn list_arch(client: &Client, username: String, dt: String) -> Result<Vec<model::Topic>> {
    let sql =
        "SELECT id, title, summary, dateline from topics WHERE writer = $1 AND is_del = false";
    let sql = format!("{} AND dateline BETWEEN '{}'::timestamp AND '{}'::timestamp + (INTERVAL '1' MONTH) - (INTERVAL '1' SECOND)", sql, &dt, &dt);
    super::query(client, &sql, &[&username]).await
}

pub async fn archive(client: &Client, username: String) -> Result<Vec<model::TopicArchive>> {
    let sql = "SELECT DISTINCT to_char(DATE_TRUNC('month', dateline), 'YYYY_MM') AS dateline FROM topics WHERE writer = $1 AND is_del = false";
    super::query(client, sql, &[&username]).await
}

pub async fn list_all(client: &Client, username: String) -> Result<Vec<model::TopicList>> {
    let sql = "SELECT id, title from topics WHERE writer = $1 AND is_del = false";
    super::query(client, sql, &[&username]).await
}
...
pub async fn content(client: &Client, id: i64) -> Result<model::TopicContent> {
    let sql = "SELECT html from topics WHERE id = $1 AND is_del = false";
...
pub async fn edit_data(client: &Client, id: i64) -> Result<model::TopicEdit> {
    let sql = "SELECT title, summary, markdown from topics WHERE id = $1 AND is_del = false";
    super::query_row_msg(client, sql, &[&id], "Data required non-existent!").await
}
